      ...
      <div className="journal-paper p-5 rounded-lg border border-parchment-200 dark:border-rpg-border shadow-sm space-y-4">
        ...
        {!isSupabaseConfigured ? (
          ...
          <div className="space-y-4 text-xs">
            {user?.is_anonymous ? (
              <div className="flex flex-col gap-2 bg-amber-50 dark:bg-amber-950/20 border border-amber-250 dark:border-amber-900 p-4 rounded text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-amber-850 dark:text-amber-300 block">Multi-Device Sync Inactive (Anonymous Mode)</span>
                  <Cloud className="text-amber-500 animate-bounce" size={20} />
                </div>
                <p className="text-[10px] text-parchment-700 dark:text-gray-400 leading-relaxed">
                  The shared account <code className="bg-amber-100 dark:bg-amber-900/40 px-1 py-0.5 rounded">explorer@questvault.com</code> failed to authenticate (likely due to an unconfirmed email in Supabase). The application has fallen back to a temporary anonymous account. Each device or browser session is currently isolated.
                </p>
              </div>
            ) : (
              <div className="flex items-center justify-between bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-250 dark:border-emerald-900 p-4 rounded text-xs">
                <div>
                  <span className="font-semibold text-emerald-800 dark:text-emerald-300 block mb-0.5">Global Shared Sync Active!</span>
                  <p className="text-[10px] text-parchment-650 dark:text-gray-400">
                    All devices share a single synchronized adventure journal.
                  </p>
                </div>
                <Cloud className="text-emerald-500 animate-pulse" size={24} />
              </div>
            )}

            <div className="bg-parchment-50 dark:bg-rpg-charcoal/30 p-4 rounded border border-parchment-200 dark:border-rpg-border/50 space-y-3">
              ...
              <p className="text-[11px] text-parchment-700 dark:text-gray-400 leading-normal">
                {user?.is_anonymous ? (
                  <span>
                    To enable sync across devices, run the SQL command in your <strong>Supabase SQL Editor</strong> to confirm the user email, or disable <strong>Confirm email</strong> under <strong>Auth &gt; Providers &gt; Email</strong> settings in the Supabase Dashboard.
                  </span>
                ) : (
                  <span>
                    Any addition, deletion, or completion update made on this device will immediately show up on any other phone, tablet, or browser accessing this link.
                  </span>
                )}
              </p>
              {user?.is_anonymous && (
                <button
                  onClick={async () => {
                    try {
                      await logout();
                      toast('Reconnecting to shared adventure vault...', 'success');
                      setTimeout(() => {
                        window.location.reload();
                      }, 1000);
                    } catch (err: any) {
                      toast(err.message || 'Failed to sync shared account', 'error');
                    }
                  }}
                  className="w-full py-1.5 bg-amber-500 hover:bg-amber-600 text-white font-serif uppercase tracking-wider text-[10px] font-bold rounded transition shadow-sm cursor-pointer"
                >
                  🔄 Force Sync Shared Account
                </button>
              )}
            </div>
          </div>
